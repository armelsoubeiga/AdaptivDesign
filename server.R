...
#################################################################################
#observe({
  #if (input$checkGroup=="simple_intervention"){
...
library(gsDesign) 
  ...
  output$pocok <- renderPlot({
    n.I <- nBinomial(input$p1, input$p2)
    x <- gsDesign(n.fix = n.I,k=input$k, test.type=2 ,alpha=input$alpha, sfu="Pocock", beta=input$power,delta=input$delta)
    plot(x, plottype=1,xlim=c(0,5),col="red")
  }) 
  
  output$of <- renderPlot({
    n.I <- nBinomial(input$p1, input$p2)
    x <- gsDesign(n.fix = n.I,k=input$k, test.type=2 ,alpha=input$alpha, sfu="OF", beta=input$power,delta=input$delta)
    plot(x, plottype=1,xlim=c(0,5),col="red")
  }) 
  
output$wt <- renderPlot({
    n.I <- nBinomial(input$p1, input$p2)
    x <- gsDesign(n.fix = n.I,k=input$k, test.type=2 ,alpha=input$alpha, sfu="WT", beta=input$power,delta=input$delta)
    plot(x, plottype=1,xlim=c(0,5),col="red")
  })

output$pocok.of.wt <- renderText({
paste("En cour de programmation")
}) 
  }
...
if(input$params=="1"){
  ...
  output$pocok <- renderPlot({
    n.I <- nBinomial(input$p1, input$p2)
    x <- gsDesign(n.fix = n.I, test.type=2 ,alpha=input$alpha, sfu="Pocock", beta=input$power,delta=input$delta)
    plot(x, plottype=2)
  }) 
  
  output$of <- renderPlot({
    n.I <- nBinomial(input$p1, input$p2)
    x <- gsDesign(n.fix = n.I, test.type=2 ,alpha=input$alpha, sfu="OF", beta=input$power,delta=input$delta)
    plot(x, plottype=2)
  }) 
  
  output$wt <- renderPlot({
    n.I <- nBinomial(input$p1, input$p2)
    x <- gsDesign(n.fix = n.I,test.type=2 ,alpha=input$alpha, sfu="WT", beta=input$power,delta=input$delta)
    plot(x, plottype=2)
  })
  output$pocok.of.wt <- renderText({
    paste("En cour de programmation")
...
observeEvent(input$input_action,{
  ...
  per_stage_sample_size_when_only_subpop_1_enrolled_adaptive_design_user_defined <- input$per_stage_sample_size_when_only_subpop_1_enrolled_adaptive_design_user_defined
  alpha_FWER_user_defined <- input$alpha_FWER_user_defined
  alpha_H0C_proportion_user_defined <- input$alpha_H0C_proportion_user_defined
  Delta <- input$Delta
  iter <- input$iter
  ...
  enrollment_rate_combined_population <- input$enrollment_rate_combined_population
  per_stage_sample_size_combined_standard_design_H0C <- input$per_stage_sample_size_combined_standard_design_H0C
  per_stage_sample_size_combined_standard_design_H01 <- input$per_stage_sample_size_combined_standard_design_H01
  subpopulation_2_stopping_boundary_proportionality_constant_adaptive_design <- input$subpopulation_2_stopping_boundary_proportionality_constant_adaptive_design
  H01_futility_boundary_proportionality_constant_adaptive_design <- input$H01_futility_boundary_proportionality_constant_adaptive_design
  H0C_futility_boundary_proportionality_constant_standard_design <- input$H0C_futility_boundary_proportionality_constant_standard_design
  H01_futility_boundary_proportionality_constant_standard_design <- input$H01_futility_boundary_proportionality_constant_standard_design
...
#####################################################
  ...
  power_curve_plot <- function(){
    plot(0,type="n",xlim=c(min(risk_difference_list),max(risk_difference_list)),ylim=c(0,1),
         main="Power versus Average Treatment Effect in Subpopulation 2",
         xlab="Avg. Treatment Effect on Risk Difference Scale in Subpopulation 2",ylab="Power")
